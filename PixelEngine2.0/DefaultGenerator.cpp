...
class DefaultGenerator : public WorldGenerator {
    int getBaseHeight(World* world, int x, Chunk* ch) {
        ...
        if(nullptr == ch) {
            return 0; // ch 값이 null pointer일 경우 종료
        }
        ...
        return 0;
        //Biome 포인터 b가 가르키는 DEFAULT, PLANS, FOREST, MOUNTAINS 각각의 기본 Biome을 생성
    }
    ...
    int getHeight(World* world, int x, Chunk* ch) {
        ...
        return baseH;
        //Biome 포인터 b가 가르키는 DEFAULT, PLANS, FOREST, MOUNTAINS 각 Biome에 따라 생성
    }
    void generateChunk(World* world, Chunk* ch) override {
        ...
        for(int x = 0; x < CHUNK_W; x++) {
            ...
            for(int y = 0; y < CHUNK_H; y++) {
                ...
                //Test_1에 해당하는 청크 단위의 Material 색상을 지정
                ...
                //Test_1_2에 해당하는 청크 단위의 Material 색상을 지정
